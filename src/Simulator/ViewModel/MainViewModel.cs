...
namespace Simulator.ViewModel
{
    ...
    public partial class MainViewModel : ObservableObject, IDisposable
    {
        ...
        public MainViewModel()
        {
            ...
            WeakReferenceMessenger.Default.Register<AssemblyFileMessage>(this, (r, m) => FileOpenedNotification(m));
            WeakReferenceMessenger.Default.Register<StateFileMessage>(this, (r, m) => StateLoadedNotifcation(m));
        ...
        private void FileOpenedNotification(AssemblyFileMessage notification)
        {
#if DEBUG
            _logger.Debug("Got AssemblyFileMessage, notfication, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            if (notification.Notification != "FileLoaded")
            {
                return;
            }

            Proc.LoadProgram(notification.Content.MemoryOffset, notification.Content.Program, notification.Content.InitialProgramCounter);
            FilePath = string.Format("Loaded Program: {0}", notification.Content.FilePath);
            OnPropertyChanged(nameof(FilePath));

            IsProgramLoaded = true;
            OnPropertyChanged(nameof(IsProgramLoaded));

            Listing = notification.Content.Listing;
            OnPropertyChanged(nameof(Listing));

            Reset();
        }

        private void StateLoadedNotifcation(StateFileMessage notification)
        {
#if DEBUG
            ...
            FilePath = string.Format("Loaded State: {0}", notification.Content.FilePath);
            OnPropertyChanged(nameof(FilePath));

            Listing = notification.Content.Listing;
            OnPropertyChanged(nameof(Listing));

            OutputLog = new MultiThreadedObservableCollection<OutputLog>(notification.Content.OutputLog);
            OnPropertyChanged(nameof(OutputLog));

            NumberOfCycles = notification.Content.NumberOfCycles;

            Proc = notification.Content.Processor;
            UpdateMemoryPage();
            UpdateUi();
        ...
        {
#if DEBUG
            _logger.Debug("Updating MemoryPage, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            MemoryPage.Clear();
        ...
        {
#if DEBUG
            _logger.Debug("Resetting, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
        ...
        {
#if DEBUG
            _logger.Debug("Step, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
        ...
        {
#if DEBUG
            _logger.Debug("UI Update, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            OnPropertyChanged(nameof(Proc));
            OnPropertyChanged(nameof(NumberOfCycles));
            OnPropertyChanged(nameof(CurrentDisassembly));
        ...
        {
#if DEBUG
            _logger.Debug("Processor Step, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            Proc.NextStep();
        ...
        {
#if DEBUG
            _logger.Debug("OutputLog, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            if (Proc.CurrentDisassembly == null)
            {
        ...
        {
#if DEBUG
            _logger.Debug("Run / Pause, at: {time}, ", DateTimeOffset.UtcNow);
#endif
        ...
        {
#if DEBUG
            _logger.Debug("Init BackGroundWorker, at: {time}, ", DateTimeOffset.UtcNow);
#endif
        ...
        {
#if DEBUG
            _logger.Debug("Is BreakPoint Triggered, at: {time}, ", DateTimeOffset.UtcNow);
#endif
        ...
        {
#if DEBUG
            _logger.Debug("Get CPU Speed: {_CpuSpeed}, at: {time}, ", CpuSpeed, DateTimeOffset.UtcNow);
#endif
            switch (CpuSpeed)
            {
                case 0:
        ...
        {
#if DEBUG
            _logger.Debug("Opening File, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
            ...
            WeakReferenceMessenger.Default.Send(new OpenFileMessage("OpenFileWindow"));
        }

        private void SaveState()
        {
#if DEBUG
            _logger.Debug("Saving State, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
            ...
            WeakReferenceMessenger.Default.Send(new StateFileMessage(new StateFileModel
                {
                    NumberOfCycles = NumberOfCycles,
                    Listing = Listing,
        ...
        {
#if DEBUG
            _logger.Debug("Adding BreakPoint, at: {time}, ", DateTimeOffset.UtcNow);
#endif
        ...
        {
#if DEBUG
            _logger.Debug("Removing BreakPoint, at: {time}, ", DateTimeOffset.UtcNow);
#endif
        ...
        {
#if DEBUG
            _logger.Debug("Sending NMI, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
        ...
        {
#if DEBUG
            _logger.Debug("Sending IRQ, at: {time}, ", DateTimeOffset.UtcNow);
#endif
            IsRunning = false;
